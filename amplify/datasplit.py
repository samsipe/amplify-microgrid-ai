...
class DataSplit:
    ...
    def _make_series(self, dataframe):
        ...
        for i in range(self.start_index, self.end_index, self.stride,):
    ...
    def _train_val_test_split(self):
        ...
        self.indices_or_sections = [int(self.train_split * self.data_array.shape[0]),
                                    int((1 - self.test_split) * self.data_array.shape[0])
                                   ]
        
        # Perform the split based on shuffled index using Numpy split
        self.train_ds, self.val_ds, self.test_ds = np.split(self.data_array, 
    ...
    def xy_splits(self, dataset):
        ...
        self.x_ds, self.y_solar, self.y_usage = self.dataset[:, :, 0:-2], self.dataset[:, :, -2], self.dataset[:, :, -1]
